...
from .srbind_data_source import (
    del_user_srbind,
    generate_qrcode,
    get_user_srbind,
    set_user_srbind,
...
async def _(bot: Bot, event: GroupMessageEvent, arg: Message = CommandArg()):
    ...
    await sruid.finish(message=msg)
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if user_id in qrbind_buffer:
        await srqr.finish("你已经在绑定中了，请扫描上一次的二维码")
    mys_api = MysApi()
    login_data = await mys_api.create_login_qr(8)
    if login_data is None:
        await srqr.finish("生成二维码失败，请稍后重试")
    ...
    qrbind_buffer[user_id]["group_id"] = event.group_id
    qrbind_buffer[user_id]["tome"] = event.is_tome()
    await srqr.send(MessageSegment.image(qr_img))
    await srqr.finish("请在3分钟内使用米游社扫码并确认进行绑定。\n注意：1.扫码即代表你同意将cookie信息授权给Bot使用\n2.扫码时会提示登录游戏，但不会挤掉账号\n3.其他人请不要乱扫，否则会将你的账号绑到TA身上！")
        ...
        for user_id, data in qrbind_buffer.items():
            ...
            try:
                ...
                if status_data is None:
                    ...
                    await cast(Bot, bot).send_group_msg(group_id=group_id, message=msg)
