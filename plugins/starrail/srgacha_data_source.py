...
from .srgacha_model import GachaLog, GachaLogItem, UserGachaLog, GachaLogResponse
...
async def get_gacha(bot_id: str, user_id: str, sr_uid: str) -> Optional[UserGachaLog]:
    ...
    try:
        ...
        logger.error(f"Error getting gacha: {e}")
        return None
...
async def save_gacha(gacha: UserGachaLog):
    ...
    try:
        ...
        if select_gacha:
            await UserGachaLog.filter(id=select_gacha.id).update(gacha=gacha.gacha)
        else:
            await gacha.save()
    except Exception as e:
        logger.error(f"Error saving gacha: {e}")
...
async def update_srgacha(bot_id: str, user_id: str, sr_uid: str, url: str) -> str:
    ...
    try:
        ...
        logger.error(f"Error updating gacha: {e}")
        return "更新抽卡记录时发生错误"
