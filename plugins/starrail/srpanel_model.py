...
sr_score_url = config_data['starrail']['sr_score_url']
sr_panel_url = config_data['starrail']['sr_panel_url']
...
class UserPanel(Model):
    ...
    cid = fields.CharField(max_length=64)
...
async def set_user_srpanel(panel: UserPanel) -> None:
    select_panel = await get_user_srpanel(panel.bot_id, panel.user_id, panel.sr_uid, panel.cid)
    if select_panel:
        select_panel.info = panel.info
        await select_panel.save()
    else:
        await panel.save()
