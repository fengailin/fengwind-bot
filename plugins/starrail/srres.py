...
async def _():
    msg = "开始更新游戏资源列表"
    await sr_update.send(msg)
    status = await srres.update()
    if not status:
        msg = "游戏资源列表更新失败，请查看控制台输出"
    else:
        msg="游戏资源列表更新完成"
    await sr_update.finish(msg)


@sr_guide_update.handle()
async def _():
    msg = "开始预下载攻略文件，可能需要较长时间"
    await sr_guide_update.send(msg)
    status = await srres.download_guide()
    if not status:
        msg = "攻略文件预下载失败，请查看控制台输出"
    else:
        msg = "攻略文件预下载完成"
    await sr_guide_update.finish(msg)


@sr_guide_force_update.handle()
async def _():
    msg = "开始强制更新攻略文件，可能需要较长时间"
    await sr_guide_force_update.send(msg)
    status = await srres.download_guide(True)
    if not status:
        msg = "攻略文件强制更新失败，请查看控制台输出"
    else:
        msg ="攻略文件强制更新完成"
    await sr_guide_force_update.finish(msg)
