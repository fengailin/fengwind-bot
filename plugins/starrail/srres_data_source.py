...
from ..config import resource_path,config_data
from .srres_model.paths import PathIndex
from .srres_model.elements import ElementIndex
from .srres_model.properties import PropertyIndex
from .srres_model.achievements import AchievementIndex
from .srres_model.light_cones import (
    LightConeIndex,
    LightConeRankIndex,
    LightConePromotionIndex,
)
from .srres_model.relics import (
    RelicIndex,
    RelicSetIndex,
    RelicSubAffixIndex,
    RelicMainAffixIndex,
)
from .srres_model.characters import (
    CharacterIndex,
    CharacterRankIndex,
    CharacterSkillIndex,
...
sr_wiki_url =config_data['starrail']['sr_wiki_url']
sr_wiki_providers =config_data['starrail']['sr_wiki_providers']
ResFiles = {
    "characters",
    "character_ranks",
    "character_skills",
...
class StarRailRes:
    ...
    def proxy_url(self, url: str) -> str:
        if config_data['starrail']['github_proxy']:
            github_proxy = config_data['starrail']['github_proxy']
    ...
    async def cache(self, file: str, refresh: bool = False):
        ...
        if not (plugin_data_dir / file).exists() or refresh:
            ...
            data = await self.download(
                self.proxy_url(f"{sr_wiki_url}/{file}")
            )
            if not data:
    ...
    async def get_character_overview(self, name: str) -> Optional[bytes]:
        ...
        if id in self.ResIndex["characters"]:
            ...
            overview = [
                n
                for n in overview
                if any(p in n for p in sr_wiki_providers)
            ]
            if overview:
    ...
    def get_character_overview_url(self, name: str) -> Optional[str]:
        ...
        if id in self.ResIndex["characters"]:
            ...
            if overview:
                if isinstance(overview, list):
                    overview = random.choice(overview)
                return self.proxy_url(f"{sr_wiki_url}/{overview}")
        return None
    ...
    async def update(self) -> bool:
        ...
        data = await self.download(
            self.proxy_url(f"{sr_wiki_url}/{VersionFile}")
        )
        if not data:
        ...
        for name in ResFiles:
            ...
            if not (index_dir / filename).exists() or update_index:
                ...
                data = await self.download(
                    self.proxy_url(
                        f"{sr_wiki_url}/index_min/cn/{filename}"
                    )
                )
                if not data:
        ...
        if not (font_dir / filename).exists():
            ...
            data = await self.download(
                self.proxy_url(f"{sr_wiki_url}/font/{filename}")
            )
            if not data:
                logger.error(f"文件 {filename} 下载失败")
                status = False
